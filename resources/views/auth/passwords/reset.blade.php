  ...
  <div class="content">
    <div class="container">
      <div class="col-md-4 ml-auto mr-auto">
        <form role="form" method="POST" action="{{ route('password.reset.store') }}">
          @csrf
          ...
          <div class="card card-login card-plain">
            ...
            <div class="card-body ">
              ...
              <div class="input-group no-border form-control-lg {{ $errors->has('email') ? ' has-danger' : '' }}">
                ...
                <input id="email" type="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ $email }}" required autocomplete="email" readonly placeholder="{{ __('Email') }}">
              </div>
              @error('email')
            ...
            <div class="card-footer ">
              <button  type = "submit" class="btn btn-primary rounded-0 btn-lg btn-block mb-3">{{ __('Reset Password') }}</button>
            </div>
          </div>
        </form>
      </div>
